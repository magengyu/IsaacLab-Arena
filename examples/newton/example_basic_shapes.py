...
class Example:
    def __init__(self, viewer, args):
        ...
        # YCB CRACKER BOX
        # The Isaac physics asset references this visual USD from a sibling
        # directory. Load its mesh directly, then reproduce the authored mass
        # with an invisible box collider for robust dynamic contact.
        ...
        self.cracker_box_pos = wp.vec3(0.0, 0.0, 0.8)
        body_cracker_box = builder.add_body(
            xform=wp.transform(p=self.cracker_box_pos, q=wp.quat_identity()),
            label="cracker_box",
        )
