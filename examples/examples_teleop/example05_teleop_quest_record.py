"""Level 5：Quest XR 遥操作 + 录制示范（GoalPoseTask 成功判定）。

在 example03（锚点修正版）基础上，把「无任务」换成「目标位姿任务」，并接入 recorder：

  - 任务：NoTask → GoalPoseTask，把方块放进目标区（target_x/y/z_range）算成功。
  - 录制：通过 env_cfg_callback 注入 ActionStateRecorderManagerCfg，逐帧把 (obs, action)
    写进 HDF5；连续 num_success_steps 步成功 → 导出该 demo → 自动 reset 录下一条。
  - 锚点：保留 example03 的 anchor_pos / anchor_rot 修正。

录出的 HDF5（data/demo_N/{actions, obs, states, ...}）可直接喂给
example07_train_bc.py 做行为克隆训练。

运行步骤同 example03（两个终端）：
  1. 终端 1：/isaac-sim/python.sh -m isaacteleop.cloudxr --host-client
  2. 终端 2：source ~/.cloudxr/run/cloudxr.env
             /isaac-sim/python.sh examples/examples_teleop/example05_teleop_quest_record.py --xr
  3. Quest 浏览器 Connect → XR 标签页 Start Session → Play
  4. 把方块放进目标区，连续成功 num_success_steps 步后自动保存并 reset。
...
def main() -> None:
    """构建 Franka 目标位姿场景，Quest 遥操作并录制成功示范。"""
    ...
    #    这样在 Quest 里能直观看到「方块要放到哪」。盒子是纯视觉 prim（无碰撞/无刚体），
    #    不影响物理仿真，也不会被 recorder 记录到 states 里。
    ...
    try:
        ...
        # 该独立示例没有实现远程 START/STOP 控制，关闭 control channel。
        ...
        with teleop_interface:
            ...
            print(
                "XR 遥操作录制已启动：移动 Quest 右手柄控制末端，Trigger 夹爪；"
                "把方块放进目标区即自动保存一条 demo。",
                flush=True,
            )
