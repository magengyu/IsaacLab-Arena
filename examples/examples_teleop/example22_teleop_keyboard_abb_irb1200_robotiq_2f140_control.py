"""Keyboard teleoperation demo for ABB IRB1200-7/0.70 with a controllable Robotiq 2F-140.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pos_sensitivity", type=float, default=0.08, help="Keyboard translation sensitivity.")
    parser.add_argument("--rot_sensitivity", type=float, default=0.10, help="Keyboard rotation sensitivity.")
    parser.add_argument(
        "--debug_joints",
        action="store_true",
...
def main() -> None:
    ...
    env_cfg = IsaacLabArenaEnvironment(
        name="teleop_abb_irb1200_keyboard_robotiq",
        embodiment=robot,
        scene=scene,
        task=NoTask(),
    ...
    print("[INFO] Building ABB IRB1200 keyboard Arena environment...")
    ...
    try:
        ...
        print(
            "ABB IRB1200 keyboard teleop started: W/S A/D Q/E translate, "
            "Z/X T/G C/V rotate, K toggles Robotiq, R resets."
        )
        ...
        while simulation_app.is_running():
            ...
            try:
                with torch.inference_mode():
                    ...
                    if step % 120 == 0:
                        print(f"[DEBUG] Keyboard 7D action: {raw_action[:7].detach().cpu().tolist()}")
...
if __name__ == "__main__":
    try:
        ...
        print("[ERROR] ABB IRB1200 keyboard teleop startup failed:")
        traceback.print_exc()
    finally:
